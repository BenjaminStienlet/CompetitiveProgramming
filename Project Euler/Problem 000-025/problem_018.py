...
from heapq import *
grid = map(lambda x: map(lambda y: int(y), x.split(' ')), open('problem_018.txt', 'r').readlines())

queue = []
smallest = (grid[0][0], (0, 0))

while True:
    (c, r) = smallest[1]
    if c != len(grid) - 1:
        heappush(queue, (smallest[0]+grid[c+1][r], (c+1, r)))
        heappush(queue, (smallest[0]+grid[c+1][r+1], (c+1, r+1)))
